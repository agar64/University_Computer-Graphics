...
pg.display.set_caption("Amogus")
m = numpy.zeros((1280, 960, 3), dtype=numpy.uint8)
surface = pg.surfarray.make_surface(m)
...
def drawRef(mat):
    pg.surfarray.blit_array(screen, mat)
    pg.display.update()
...
def setPixelMono(matrix, x, y, color):
    matrix[x,y] = color
    return matrix

def drawing(buf):
    for x in range (0, buf.shape[0]-1):
        y = (buf.shape[1]/2) + 5*numpy.sin(x*0.2)
        y = y.astype(numpy.int32)
        buf = setPixel(buf, x, y, 255, 255, 255)
    return buf

def bresenham(buf, xi, yi, xf, yf, R, G, B):
    dx = xf - xi
    dy = yf - yi
    ...
    for i in range (0, numpy.absolute(dx)):
...
def bres(buf, xi, yi, xf, yf, R, G, B):
    dx = numpy.absolute(xf - xi)
    dy = numpy.absolute(yf - yi)
    
    if(numpy.absolute(dx) < numpy.absolute(dy)):
...
def bres2(buf, xi, yi, xf, yf, R, G, B):
    dx = numpy.absolute(xf - xi)
    dy = numpy.absolute(yf - yi)
    decide = 0
    if(dx < dy):
...
for i in range(0, 3600):
    m = bres2(m, 500, 500, 800-(100*numpy.sin(i*numpy.pi/180.)).astype(numpy.uint8), 500+(100*numpy.sin(i*numpy.pi/180.)).astype(numpy.uint8), 255, 255, 255)
    draw()
    
100*numpy.sin(i*numpy.pi/180.)

for i in range(0, 360):
    m = bres2(m, 500, 500, 500+(100*numpy.cos(i*numpy.pi/180.)).astype(numpy.uint8), 500+(100*numpy.sin(i*numpy.pi/180.)).astype(numpy.uint8), 255, 255, 255)
    draw()
...
def checkLine():
    for i in range(400, -400, -1):
        for j in range(-400, 400,):
            m = DDA(m, 500, 500, 500+i, 500+j, 255, 255, 255)
            draw()

def checkLine():
    for i in range(-400, 400):
        m = DDA(m, 500, 500, 900, 500+i, 255, 255, 255)            
        draw()
    for i in range(400, -400, -1):
        m = DDA(m, 500, 500, 500+i, 900, 255, 255, 255)            
        draw()
    for i in range(400, -400, -1):
        m = DDA(m, 500, 500, 100, 500+i, 255, 255, 255)            
        draw()
    for i in range(-400, 400):
        m = DDA(m, 500, 500, 500+i, 100, 255, 255, 255)            
        draw()
...
def bres4(buf, xi, yi, xf, yf, R, G, B):
    dx = numpy.absolute(xf - xi)
    dy = numpy.absolute(yf - yi)
    x = round(xi)
    y = round(yi)
...
def bresCirc(buf, xi, yi, xf, yf, R, G, B):
    dx = numpy.absolute(xf - xi)
    dy = numpy.absolute(yf - yi)
    radius = numpy.sqrt(pow((xi - xf), 2) + pow((yi - yf), 2))
...
def bresCirc2(buf, xi, yi, xf, yf, R, G, B):
    cx = xi
    cy = yi
    x = xf
    y = yf
    radius = numpy.sqrt(pow((x - cx), 2) + pow((y - cy), 2))

    x = 0
    y = int(radius)
    p = 3 - 2*radius
    buf = setPixel(buf, cx+x, cy+y, R, G, B)
    buf = setPixel(buf, cy+y, cx+x, R, G, B)
    buf = setPixel(buf, cy+y, cx-x, R, G, B)
    buf = setPixel(buf, cx+x, cy-y, R, G, B)
    buf = setPixel(buf, cx-x, cy-y, R, G, B)
    buf = setPixel(buf, cy-y, cx-x, R, G, B)
...
def floodFill(buf, x, y, corn, cori):
    if(not(numpy.array_equal(getPixelM(buf, x, y), cori))):
        return buf
    buf = setPixel(buf, x, y, corn[0], corn[1], corn[2])
    floodFill(buf, x, y-1, corn, cori)
    floodFill(buf, x+1, y, corn, cori)
    floodFill(buf, x, y+1, corn, cori)
    floodFill(buf, x-1, y, corn, cori)

def floodFill2(buf, x, y, corn, cori):
    if(not(numpy.array_equal(getPixelM(buf, x, y), cori))):
        return buf
    while(numpy.array_equal(getPixelM(buf, x, y), cori)):
        buf = setPixel(buf, x, y, corn[0], corn[1], corn[2])
        floodFill2(buf, x, y-1, corn, cori)
        floodFill2(buf, x, y+1, corn, cori)
        x += 1
    buf = setPixel(buf, x, y, corn[0], corn[1], corn[2])
    return buf
...
tex = iio.imread(r'C:\Users\agar32\Desktop\Important Stuff\CG Projeto I\gato.jpg') #imageIO

def loadTex(location):
    tex = iio.imread(location)
    tex = numpy.rot90(tex)
    tex = numpy.flip(tex, 0)
    return tex


for z in range(0, 1):
    poliAna = addPoint(poliAna, [400, 400, 0, 0])
    poliAna = addPoint(poliAna, [600, 400, 1, 0])
    poliAna = addPoint(poliAna, [600, 700, 1, 1])
    poliAna = addPoint(poliAna, [400, 700, 0, 1])

#faster default test shape
for z in range(0, 1):
    for i in range(-400, 400):
        m = bres6(m, 500, 500, 900, 500+i, 255, 255, 255)
        for event in pg.event.get():
...
def translate(poli, value):
    for i in range(0, len(poli)):
        poli[i, 0] = poli2[i, 0] + int(value[0])
        poli[i, 1] = poli2[i, 1] + int(value[1])

def scale(poli, valueX, valueY):
    for i in range(0, len(poli)):
        poli[i, 0] = int(round(poli2[i, 0] * valueX))
        poli[i, 1] = int(round(poli2[i, 1] * valueY))

def movTest():
    for z0 in range(0, 1):
        for z in range(1, 80):
            translate(poli2, [10, 0])
            clearS()
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    pg.quit()
                    raise SystemExit
            scanLine(m, poli2, cRika)
        for z in range(1, 60):
            translate(poli2, [0, 10])
            clearS()
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    pg.quit()
...
#rotação no lugar = matRotF = numpy.matmul(toOrigin, numpy.matmul(matRot, toOriginMinus))
